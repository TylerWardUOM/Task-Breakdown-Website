...
export const useApiWrapper = () => {
  ...
  return {
    ...
    deleteTaskRequest: (taskId: number): Promise<void> => apiCall(() => deleteTaskRequest(taskId)),
    saveTask: (taskData: Partial<Task_data>, existingTask?: Task | null): Promise<Task> =>
      apiCall(() => saveTask(taskData, existingTask)),
    registerUserInDatabase: (email: string, username: string): Promise<{ success: boolean; message?: string; errorCode?: string; errorMessage?: string }> =>
      apiCall(() => registerUserInDatabase(email, username)),
    markUserAsVerified: (email: string): Promise<void> => apiCall(() => markUserAsVerified(email)),
    ...
    fetchSubtaskById: (subtaskId: string): Promise<Subtask> =>
    ...
    toggleSubtaskCompletionRequest: (subtaskId: number, isCompleted: boolean | null): Promise<Subtask> =>
      apiCall(() => toggleSubtaskCompletionRequest(subtaskId, isCompleted)),
    deleteSubtask: (subtaskId: number): Promise<void> => apiCall(() => deleteSubtask(subtaskId)),
  };
};
